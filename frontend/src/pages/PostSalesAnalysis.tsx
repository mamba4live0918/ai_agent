  ...
  return (
    <div className="flex h-full relative">
      ...
      <div className={`absolute left-0 top-0 bottom-0 z-20 flex flex-row
        ...
        {/* Sidebar header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--bg-primary)]">
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map(s => (
              <div
                ...
                className={`relative group px-4 py-3 cursor-pointer border-b border-[var(--bg-primary)] hover:bg-[var(--bg-secondary)] transition-colors ${
        ...
        <div className="px-4 py-2 border-t border-[var(--bg-primary)]">
          <button
            onClick={() => { handleCreate(); setSidebarOpen(false); }}
            disabled={creating}
    ...
    {/* Backdrop overlay when sidebar is open */}
      ...
      <div className={`flex-1 min-w-0 bg-[var(--bg-primary)] flex flex-col transition-[margin] duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${sidebarOpen ? 'ml-[260px] sm:ml-[280px]' : 'ml-0'}`}>
        {/* Header bar */}
        <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 border-b border-[var(--bg-primary)]">
          <span className="text-sm font-semibold text-[var(--text-primary)]">售后分析</span>
