    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-6 sm:py-10">
      ...
      <div className="mb-10 animate-in" style={{ animationDelay: '0ms' }}>
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--btn-primary)] to-[var(--btn-primary-hover)] flex items-center justify-center shadow-glow-green">
            <svg className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <div>
            <h2 className="text-2xl font-bold text-[var(--text-primary)]">销售辅助平台</h2>
            <p className="text-sm text-[var(--text-secondary)] mt-0.5">AI 驱动的销售全流程辅助与客户分析系统</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8 animate-in" style={{ animationDelay: '80ms' }}>
        <div className="card p-6 rounded-2xl transition-all duration-200 hover:-translate-y-0.5">
          ...
          <p className="text-3xl font-bold text-[var(--accent-blue)] font-mono tabular-nums">
            {loading ? '--' : docCount}
          </p>
        ...
        <div className="card p-6 rounded-2xl transition-all duration-200 hover:-translate-y-0.5">
          ...
          <p className="text-3xl font-bold text-[var(--accent-green)] font-mono tabular-nums">
        ...
        <div className="card p-6 rounded-2xl transition-all duration-200 hover:-translate-y-0.5">
          ...
          <p className="text-3xl font-bold text-[var(--accent-orange)] font-mono tabular-nums">
            {loading ? '--' : productCount}
          </p>
          <p className="text-xs text-[var(--text-tertiary)] mt-1">基金/保险/信托/理财产品</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in" style={{ animationDelay: '150ms' }}>
        <Link to="/knowledge" className="card card-hover p-6 rounded-xl group transition-all duration-200">
          ...
          <h3 className="text-base font-semibold text-[var(--text-primary)] mb-2">知识库</h3>
          <p className="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">上传文档自动索引，RAG 智能问答，AI 生成优先参考知识库</p>
        ...
        <Link to="/customers" className="card card-hover p-6 rounded-xl group transition-all duration-200">
          ...
          <h3 className="text-base font-semibold text-[var(--text-primary)] mb-2">客户分析</h3>
          <p className="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">AI 6 维画像评分、售前准备报告、资产配置方案、PDF 导出</p>
          <div className="flex items-center gap-2">
        ...
        <Link to="/products" className="card card-hover p-6 rounded-xl group transition-all duration-200">
          ...
          <h3 className="text-base font-semibold text-[var(--text-primary)] mb-2">产品库</h3>
          <p className="text-sm text-[var(--text-secondary)] mb-4 leading-relaxed">管理金融产品，基金自动拉取东方财富真实净值，每 4 小时刷新</p>
          <div className="flex items-center gap-2">
