...
- 分组管理合并到用户管理页面（Tab 切换：「用户列表」/「分组管理」）
- 分组卡片网格展示，点击弹窗查看详情、管理成员
- 成员表格：用户名、角色、邮箱、加入时间、订阅方案（默认免费）、剩余时间
- 添加成员：下拉选择用户，支持搜索过滤
- 移除独立 `/admin/groups` 路由和侧边栏入口

### 教师学员管理系统

- **独立页面** `/instructor/students`（Tab：「学员列表」/「学员小组」）
- 教师可认领未分组销售为自己的学员，加入指定小组
- 认领后其他教师不可再认领，释放后恢复未分组
- 教师可创建/编辑/删除自己的学员小组，管理组成员
- 后端端点：`POST /instructor/students/claim/{userId}`、`POST /instructor/students/release/{userId}`
- 讲师端口训练统计按教师管理的所有小组过滤

### 个人资料页

- **路由**：`/profile`（自己的）、`/profile/:userId`（查看他人）
- **功能**：头像上传、个性签名（100字）、状态切换（🟢在线/⏳忙/📚学习/🎣摸鱼/💼会议/☕休息/✈️出差/🏖️休假）
- **状态支持**：点击状态区域直接切换，无需进入编辑模式，自动保存
- **侧边栏**：头像点击跳转个人页，显示状态图标
- **教师**：个人页显示管理的学员小组卡片，点击查看成员，再点成员跳转主页
- 普通用户查看他人主页时不显示小组信息
