...
**安全加固**
- ChromaDB 向量检索按用户隔离（共享文档 + 自有文档），防止跨用户数据泄露
- JWT 密钥启动时自动检测，使用默认值会生成随机密钥并警告
- 审计日志表（`audit_logs`）记录关键操作（登录/文档/客户），append-only 不可篡改
- API 速率限制：登录 5次/分钟，全局 60次/分钟
- 文件加密存储预留：`FILE_ENCRYPTION_KEY` 环境变量配置
