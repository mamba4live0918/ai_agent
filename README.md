...
用户管理页面整合了分组管理（Tab 切换），不再有独立的分组管理页面。
...
**用户分组管理（已整合到用户管理页面）**
- Tab 切换「用户列表」/「分组管理」，分组卡片网格展示，点击弹窗管理成员
- 成员列表表格：用户名、角色、邮箱、加入时间、订阅方案（默认免费）、剩余时间
- 超级管理员可创建/编辑/删除分组，添加/移除成员（下拉选择用户）
- 分组管理员只能管理自己组内的成员
...
- 客户详情改为居中毛玻璃浮窗，Tab 重组：客户信息 → AI 分析 → 售前准备 → 配置方案
- 编辑资料弹窗：六大类分组表单，所有字段可编辑保存
- CSV 批量导入客户：`POST /api/customers/import-csv`
- AI 重新生成支持从 structured_data 自动构建描述
- 联系方式脱敏显示，点击「查看」完整展示
